...
def heuristic_parse_condition(text: str) -> dict:
    """LLM을 쓸 수 없을 때 핵심 한국어 표현만 보수적으로 해석한다.

    Compose의 기본값은 LLM 비활성이다. 이때 모든 요청을 1매로 고정하면 화면에서
    "2명 연석"을 신청해도 1석만 배정되는 정합성 오류가 생긴다. 하드 조건은
    단호한 표현이 있을 때만 만들고, 나머지는 선호로 두어 매칭을 과도하게 막지 않는다.
    """
    ...
    count_match = re.search(r"(\d+)\s*(?:명|장|매|자리|석)", lowered)
    word_count_match = re.search(r"(한|두|세|네)\s*(?:명|장|매|자리|석)", lowered)
    if count_match:
        required["count"] = min(4, max(1, int(count_match.group(1))))
    elif word_count_match:
        required["count"] = {"한": 1, "두": 2, "세": 3, "네": 4}[word_count_match.group(1)]
    else:
        required["count"] = DEFAULT_COUNT

    grades = [
        grade for grade in ("VIP", "R", "S", "A")
        if re.search(rf"(?<![A-Z]){grade}\s*(?:석|등급)", source, re.IGNORECASE)
    ]
    strict = bool(re.search(r"무조건|반드시|꼭|아니면\s*(?:안|못)", source))
    if grades:
        (required if strict else preferred)["grade"] = grades

    rows = [
        row.upper() for row in re.findall(r"(?<![A-Za-z])([A-R])\s*열", source, re.IGNORECASE)
        if row.upper() in VALID_ROWS
    ]
    if rows:
        (required if strict else preferred)["row"] = list(dict.fromkeys(rows))

    hard_price = re.search(r"(\d[\d,]*)\s*만\s*원?\s*(?:이하|아래|까지|넘지)", lowered)
    soft_price = re.search(r"(\d[\d,]*)\s*만\s*원?\s*(?:정도|쯤|선)", lowered)
    if hard_price:
        required["max_price"] = int(hard_price.group(1).replace(",", "")) * 10000
    elif soft_price:
        amount = int(soft_price.group(1).replace(",", "")) * 10000
        preferred["max_price"] = amount
        flexible["price_ceiling"] = round(amount * 1.15)

    wants_together = bool(re.search(r"붙어|나란히|연석|같이\s*앉", source))
    if wants_together:
        (required if strict else preferred)["consecutive"] = True

    if re.search(r"떨어져|따로\s*앉|나눠\s*앉", source):
        flexible["allow_split"] = True
        gap = re.search(r"(\d+)\s*칸", source)
        flexible["max_split_gap"] = int(gap.group(1)) if gap else DEFAULT_SPLIT_GAP
    ...
    logger.info(f"[AI] 규칙 기반 조건 파싱 - required: {parsed['required']}")
    return parsed
...
async def parse_condition(text: str) -> dict:
    ...
    폴백: 인원·등급·열·가격·연석/분리 허용의 핵심 표현을 규칙으로 해석한다.
    """
    ...
    if content is None:
        return heuristic_parse_condition(text)
