# 공연 예매 플랫폼 API 명세서 (백엔드 → 프론트엔드) v2

`API_SPEC.md`(v1)에서 예매 취소·취소표 대기·정원(매진) 관련 내용 추가하고 발견된
버그 수정 사항까지 반영한 최신 버전. v1은 남겨두되 이 문서 기준으로 볼 것.
...
**취소한 공연은 다시 예매할 수 있음.** (초기 버전엔 DB 제약 때문에 한 번 취소하면 그
공연을 영원히 재예매 못 하는 버그가 있었는데 수정함 — 실제 재현 테스트로 확인)
...
이전에 대기 등록했다가 매칭(`MATCHED`)된 이력이 있어도, 그 공연이 다시 매진되면
재등록 가능함(중복 체크는 현재 `WAITING` 상태인 것만 봄).
...
### GET /api/recommend/forecast/{courseId} — 수요예측 (스텁, 미완성)
라우팅만 확보해둔 상태. 실제 계산 로직·응답 필드 구조는 아직 없음(AI 담당 작업 예정).

Response (200) — 지금은 이 형태로만 응답함, 나중에 필드 완전히 바뀔 수 있음
```json
{ "courseId": 4, "message": "TODO: AI 담당 구현 예정" }
```
...
- **recommend-service 응답의 `price`가 문자열로 옴.** `GET /api/recommend/{userId}`,
  `GET /api/recommend/forecast/{courseId}` 응답 안의 `price` 필드가 `"30000.0"`처럼
  따옴표 있는 문자열로 내려옴. `course-service`(`GET /api/courses` 등)나
  `payment-service`(`amount`)는 숫자(`55000.00`, 따옴표 없음)로 내려오는 것과 다름 —
  Python(FastAPI) 쪽 Decimal 타입의 기본 직렬화 방식 때문. 화면에서 숫자 연산/포맷팅
  (`toLocaleString()` 등) 하려면 `Number(price)`로 변환하고 쓸 것. recommend-service
  담당 확인 필요, 백엔드(Java)는 건드리지 않음.
...
- **매진된 공연에 예매가 안 됨** → 정상 동작. `capacity`가 설정된 공연은 `enrollmentCount >= capacity`가 되면 예매 대신 `POST /api/enrollments/waitlist`로 대기 등록해야 함
